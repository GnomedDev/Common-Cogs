...
class Gnome(commands.Cog):
    ...
    async def donate(self, ctx):
        await ctx.send(
            f"To donate to support the development and hosting of {self.bot.user.mention}, you can donate via Patreon!\nhttps://www.patreon.com/Gnome_the_Bot_Maker")
    ...
    async def botstats(self, ctx):
      ...
      {self.bot.user.name} can be used by {sum([guild.member_count for guild in self.bot.guilds]):,} people"""
        ...
        embed = discord.Embed(title=f"{self.bot.user.name} Stats", description=message,
                              url="https://discord.gg/zWPWwQC", color=0x5bc0ec)
    ...
    async def suggest(self, ctx, *, suggestion):
        await self.bot.get_channel(696325283296444498).send(
            f"{str(ctx.author)} in {ctx.guild.name} suggested: {suggestion}")
        ...
        for member in targetguild.members:
            ...
            if role.id in member_roles:
                if member.id not in owner_id_list:
                    ...
                    embed = discord.Embed(
                        description=f"Role Removed: Removed {role.mention} from {owner.mention}\n**Reason:** No longer Owner of a Server")
                ...
                if member.id in owner_id_list:
                    ...
                    embed = discord.Embed(
                        description=f"Role Added: Gave {role.mention} to {owner.mention}\n**Reason:** Owner of a Server but not added by on_guild_join")
                    embed.set_author(name=f"{str(owner)} (ID {owner.id})", icon_url=owner.avatar_url)
                    embed.set_thumbnail(url=owner.avatar_url)
            ...
            for a_id in (738009431052386304, 738009620601241651, 738009624443224195):
                if a_id in member_roles and 703307566654160969 not in member_roles:
                    ...
                    await self.bot.get_channel(696347411966066689).send(
                        f"Added highlighted owner of a server to {member.mention}")
                    break
        await ctx.send("Done!")
